...
def setup_pump_curve_model(
    inlet_flow_mol=44.5612,     # mol/s (~ 2.9 m³/h)
    inlet_pressure=1e5,         # Pa
    inlet_temperature=300,       # K
    plot_curves=True,            # Generate performance plot
    save_plot_path=None,         # Path to save plot (optional)
    ...
    """Creates and solves a pump model with performance curves"""
    # Validate input arrays
    if len(flow_data_m3h) != len(head_data_m) or len(flow_data_m3h) != len(eff_data):
        raise ValueError("flow_data_m3h, head_data_m and eff_data must have the same length")
    ...
    def pump_perf_callback(blk):
        ...
        # Variables
        blk.flow_mol = Var(initialize=inlet_flow_mol, bounds=(0, 100), units=units.mol/units.s)
        blk.head_interp = Var(initialize=2500,  bounds=(0, 3000), units=units.J/units.kg)
        blk.eff_interp = Var(initialize=0.7, bounds=(0, 0.75), units=units.dimensionless)
    ...
    m.fs.unit.inlet.flow_mol[0].fix(inlet_flow_mol)
    hin = iapws95.htpx(T=inlet_temperature*units.K, P=inlet_pressure*units.Pa)
    ...
    try:
        ...
        if results.solver.termination_condition == 'optimal':
            print("\nSolved without initializer:")
            print(f"Outlet pressure: {value(m.fs.unit.outlet.pressure[0]/1e5):.1f} bar")

    # Generate performance plot if requested
    if plot_curves and results and results.solver.termination_condition == 'optimal':
        ...
        # Add operating point
        op_flow = value(m.fs.unit.inlet.flow_mol[0]) * water_mw / water_density * 3600
        ...
        if save_plot_path:
            ...
            print(f"Pump curves saved to '{save_plot_path}'")
        else:
            plt.show()
